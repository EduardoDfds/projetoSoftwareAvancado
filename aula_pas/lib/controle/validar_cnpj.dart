class ValidarCnpj {
  ...
  bool validarQuantidadeCaractere(String cnpj){

    if(cnpj.length != 18) throw Exception('CNPJ menor que 18');
    return true;
  }

  bool validaFormato(String cnpj) {
    var regExp = RegExp(r'\d{2}\.\d{3}\.\d{3}\/\d{4}\-\d{2}');
    if(!regExp.hasMatch(cnpj)) throw Exception ('CNPJ deve possuir o formato basico');
  ...
  List<int> gerarListaNumero(String cnpj) {
    ...
    List<int> listaNumerosCNPJ = cnpj.substring(0, 12).split('').map<int>((e) => int.parse(e)).toList();
    
    return listaNumerosCNPJ;
  }

  int cauculaPrimeiroDigito(String cnpj) {
    var peso = 5;
    ...
    var cnpjSemMascara = cnpj.replaceAll('.', '').replaceAll('/', '').replaceAll('-', '');
  ...
  int cauculaSegundoDigito(String cnpj) {
    
    List<int> listaNumerosCNPJ = gerarListaNumero(cnpj);
    listaNumerosCNPJ.add(cauculaPrimeiroDigito(cnpj));
  ...
  bool validarDigitos(String cnpj) {
    ...
    if (primeiroDigito != cauculaPrimeiroDigito(cnpj))
      throw Exception('Primeiro digito incorreto') ;
    if (segundoDigito != cauculaSegundoDigito(cnpj))
